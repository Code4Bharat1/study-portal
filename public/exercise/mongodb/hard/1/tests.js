...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_hard_1.json');
const PASS_FILE = path.join(__dirname, 'passed_hard_1.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Check each returned document has nested field matching criteria
    // For example, check nested property 'profile.age' exists and is > 18
    for (const doc of results) {
      if (!doc.profile || typeof doc.profile.age !== 'number' || doc.profile.age <= 18) {
        throw new Error("Document does not match nested filter criteria.");
      }
    }
