...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_hard_3.json');
const PASS_FILE = path.join(__dirname, 'passed_hard_3.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Verify documents match compound filter criteria, for example:
    // status is "shipped" AND total > 100
    for (const doc of results) {
      if (doc.status !== "shipped" || !(doc.total > 100)) {
        throw new Error("Document does not match compound filter criteria.");
      }
    }
