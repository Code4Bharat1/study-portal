...
async function runTest() {
  ...
  await clearCollection(db, 'user_logs');

  const code = fs.readFileSync(USER_SCRIPT_PATH, 'utf8');

  try {
    const runUserCode = new Function('db', code);
    const logs = await runUserCode(db);

    if (!Array.isArray(logs))
      throw new Error("Expected an array of user log entries.");

    // Example validation: check logs have 'userId' and 'action' properties
    for (const log of logs) {
      if (!log.userId || !log.action) {
        throw new Error("Log entry missing 'userId' or 'action' property.");
      }
    }
