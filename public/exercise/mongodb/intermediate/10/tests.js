...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_intermediate_10.json');
const PASS_FILE = path.join(__dirname, 'passed_intermediate_10.txt');
...
async function runTest() {
  ...
  try {
    ...
    const distinctValues = await runUserCode(db);

    if (!Array.isArray(distinctValues))
      throw new Error("Expected function to return an array.");

    if (distinctValues.length === 0)
      throw new Error("Distinct array is empty.");

    // Check for duplicates (should be distinct)
    const set = new Set(distinctValues);
    if (set.size !== distinctValues.length)
      throw new Error("Distinct array contains duplicates.");
