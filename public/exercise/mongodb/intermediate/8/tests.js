...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_intermediate_8.json');
const PASS_FILE = path.join(__dirname, 'passed_intermediate_8.txt');
...
async function runTest() {
  ...
  try {
    ...
    if (results.length < 2)
      throw new Error("Returned array must contain at least 2 documents.");

    // Check sorted ascending by price
    for (let i = 1; i < results.length; i++) {
      if (results[i].price < results[i - 1].price) {
        throw new Error("Results are not sorted ascending by price.");
      }
    }
