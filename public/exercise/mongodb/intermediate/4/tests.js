...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_intermediate_4.json');
...
async function runTest() {
  ...
  try {
    ...
    // Verify no document has 'oldtag' in the 'tags' array
    const doc = await new Promise((resolve, reject) => {
      users.findOne({ tags: 'oldtag' }, (err, doc) => {
        if (err) reject(err);
        else resolve(doc);
      });
    });
    ...
    if (doc)
      throw new Error("Found a document that still has 'oldtag' in 'tags' array.");
