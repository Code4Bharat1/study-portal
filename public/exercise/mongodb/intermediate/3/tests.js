...
async function runTest() {
  ...
  try {
    ...
    // Find a document where the array field 'tags' includes 'newtag'
    const doc = await new Promise((resolve, reject) => {
      users.findOne({ tags: 'newtag' }, (err, doc) => {
        if (err) reject(err);
        else resolve(doc);
      });
    });
    ...
    if (!doc)
      throw new Error("No document found with 'newtag' pushed into 'tags' array.");
