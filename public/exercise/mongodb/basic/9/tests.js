...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_basic_9.json');
...
async function runTest() {
  ...
  await clearCollection(db, 'people');
  ...
  try {
    ...
    if (results.length === 0) throw new Error("Returned array is empty.");

    for (let i = 1; i < results.length; i++) {
      if (results[i].age > results[i - 1].age)
        throw new Error("Array is not sorted by age descending.");
    }
