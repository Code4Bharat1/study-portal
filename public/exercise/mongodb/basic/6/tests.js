...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_basic_6.json');
...
async function runTest() {
  ...
  try {
    ...
    const updatedDoc = await new Promise((resolve, reject) => {
      users.findOne({ name: "Updated Name" }, (err, doc) => {
        if (err) reject(new Error("Error querying 'users' collection."));
        else resolve(doc);
      });
    });

    if (!updatedDoc) throw new Error("No document found with name 'Updated Name'.");
