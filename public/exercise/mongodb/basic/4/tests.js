...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_basic_4.json');
  ...
  try {
    ...
    const docs = await new Promise((resolve, reject) => {
      users.find({}).toArray((err, docs) => {
        if (err) reject(new Error("Error finding documents in 'users'."));
        else resolve(docs);
      });
    });

    if (!Array.isArray(docs) || docs.length === 0) throw new Error("No documents found in 'users' collection.");
