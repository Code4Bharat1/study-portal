...
async function runTest() {
  ...
  try {
    ...
    const inactiveDoc = await new Promise((resolve, reject) => {
      users.findOne({ active: false }, (err, doc) => {
        if (err) reject(new Error("Error querying 'users' collection."));
        else resolve(doc);
      });
    });

    if (inactiveDoc) throw new Error("Found document with active: false, expected none.");
