...
const server = setupServer(
  rest.get('/api/serverless', (req, res, ctx) => {
    return res(ctx.json({ message: 'Serverless Response' }));
  })
);
...
async function functionalVerify() {
  ...
  try {
    ...
    if (!res.status.mock.calls[0] || res.status.mock.calls[0][0] !== 200) {
      console.log('✘ API did not return status 200');
      allPassed = false;
    } else {
      console.log('✔ API returned status 200');
    }
    if (!res.json.mock.calls[0] || res.json.mock.calls[0][0].message !== 'Serverless Response') {
    ...
    if (allPassed) {
      console.log('\n🎉 Success! Serverless Deployment behavior is correct.');
    } else {
      console.log('\n❗ Serverless Deployment behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
