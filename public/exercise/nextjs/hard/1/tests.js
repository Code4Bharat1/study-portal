  ...
  try {
    ...
    traverse(ast, {
      ...
      ExportDefaultDeclaration(path) {
        if (path.node.declaration && path.node.declaration.async) {
          asyncFunctions++;
        }
      },
    });
    ...
    if (asyncFunctions < 1) {
      console.log('✘ No async functions found for server component');
      allPassed = false;
    } else {
...
async function functionalVerify() {
  ...
  try {
    ...
    if (data.textContent !== 'Server Component Data') {
      console.log('✘ Server component data is not rendered');
      allPassed = false;
    } else {
      console.log('✔ Server component data is rendered');
    }

    if (allPassed) {
      console.log('\n🎉 Success! App Router behavior is correct.');
    } else {
      console.log('\n❗ App Router behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
