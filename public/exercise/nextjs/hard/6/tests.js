...
function codeVerify() {
  ...
  try {
    ...
    traverse(ast, {
      CallExpression(path) {
        if (path.node.callee && path.node.callee.name === 'withFederatedModules') {
          federationCalls++;
        }
      },
    });

    if (federationCalls === 0) {
      console.log('✘ No withFederatedModules calls found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${federationCalls} withFederatedModules call(s)`);
    }
...
async function functionalVerify() {
  let allPassed = true;
  try {
    const Component = (await import('./components/FederatedComponent')).default;

    render(<Component />);
    const federatedText = screen.getByTestId('federated');
    if (federatedText.textContent !== 'Federated Content') {
      console.log('✘ Federated component is not rendered');
      allPassed = false;
    } else {
      console.log('✔ Federated component is rendered');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Micro-Frontends behavior is correct.');
    } else {
      console.log('\n❗ Micro-Frontends behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
