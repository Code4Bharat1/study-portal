...
function codeVerify() {
  ...
  try {
    ...
    let manifestMetaTags = 0;

    traverse(ast, {
      JSXElement(path) {
        if (
          path.node.openingElement.name.name === 'meta' &&
          path.node.openingElement.attributes.some(
            (attr) => attr.name.name === 'name' && attr.value.value === 'manifest'
          )
        ) {
          manifestMetaTags++;
        }
    ...
    if (manifestMetaTags === 0) {
      console.log('✘ No manifest meta tag found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${manifestMetaTags} manifest meta tag(s)`);
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    render(<Component />);
    const manifestMeta = document.querySelector('meta[name="manifest"]');
    if (!manifestMeta || manifestMeta.getAttribute('content') !== '/manifest.json') {
      console.log('✘ Manifest meta tag is not rendered');
      allPassed = false;
    } else {
      console.log('✔ Manifest meta tag is rendered');
    }

    if (allPassed) {
      console.log('\n🎉 Success! PWA behavior is correct.');
    } else {
      console.log('\n❗ PWA behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
