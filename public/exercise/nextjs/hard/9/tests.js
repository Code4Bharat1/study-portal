...
const { GET_DATA } = require('./script.js');
...
const mocks = [
  {
    ...
    result: { data: { data: { message: 'GraphQL Data' } } },
  },
];
...
function codeVerify() {
  ...
  try {
    ...
    traverse(ast, {
      CallExpression(path) {
        if (path.node.callee && path.node.callee.name === 'gql') {
          gqlCalls++;
        }
      },
    });
    ...
    if (gqlCalls === 0) {
      console.log('✘ No gql calls found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${gqlCalls} gql call(s)`);
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    if (dataText.textContent !== 'GraphQL Data') {
      console.log('✘ GraphQL data is not rendered');
      allPassed = false;
    } else {
      console.log('✔ GraphQL data is rendered');
    }

    if (allPassed) {
      console.log('\n🎉 Success! GraphQL behavior is correct.');
    } else {
      console.log('\n❗ GraphQL behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
