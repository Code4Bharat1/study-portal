...
function codeVerify() {
  ...
  try {
    ...
    traverse(ast, {
      CallExpression(path) {
        if (path.node.callee.name === 'useSession') {
          useSessionCalls++;
        }
      },
    });

    if (useSessionCalls === 0) {
      console.log('✘ No useSession calls found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${useSessionCalls} useSession call(s)`);
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    const sessionText = screen.getByTestId('session');
    if (sessionText.textContent !== 'Welcome, Test User') {
      console.log('✘ Session message is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ Session message is rendered correctly');
    }
    ...
    const noSessionText = screen.getByTestId('session');
    if (noSessionText.textContent !== 'Please sign in') {
      console.log('✘ No-session message is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ No-session message is rendered correctly');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Authentication behavior is correct.');
    } else {
      console.log('\n❗ Authentication behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
