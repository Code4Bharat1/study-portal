...
async function functionalVerify() {
  ...
  try {
    ...
    render(<Component />);
    const message = screen.getByTestId('deploy-message');
    if (message.textContent !== 'Ready for Deployment') {
      console.log('✘ Deployment message is not correct');
      allPassed = false;
    } else {
      console.log('✔ Deployment message is correct');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Deploying Next.js behavior is correct.');
    } else {
      console.log('\n❗ Deploying Next.js behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
