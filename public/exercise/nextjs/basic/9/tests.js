...
function codeVerify() {
  ...
  try {
    ...
    if (envAccess === 0) {
      console.log('✘ No process.env access found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${envAccess} process.env access(es)`);
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    process.env.NEXT_PUBLIC_API_KEY = 'abc123';
    render(<Component />);

    const apiKey = screen.getByTestId('api-key');
    if (apiKey.textContent !== 'API Key: abc123') {
      console.log('✘ API key is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ API key is rendered correctly');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Environment Variables behavior is correct.');
    } else {
      console.log('\n❗ Environment Variables behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
