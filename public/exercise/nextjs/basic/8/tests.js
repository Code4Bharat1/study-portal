...
function codeVerify() {
  ...
  try {
    ...
    if (imageElements === 0) {
      ...
      console.log(`✔ Found ${imageElements} Image component(s)`);
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    if (image.tagName.toLowerCase() !== 'img') {
      console.log('✘ Image component is not rendered as an img tag');
      allPassed = false;
    } else {
      console.log('✔ Image component is rendered as an img tag');
    }

    if (image.getAttribute('alt') !== 'Test Image') {
      console.log('✘ Image alt text is not correct');
      allPassed = false;
    } else {
      console.log('✔ Image alt text is correct');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Image Optimization behavior is correct.');
    } else {
      console.log('\n❗ Image Optimization behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
