...
function codeVerify() {
  ...
  try {
    ...
    if (lazyCalls === 0) {
      console.log('✘ No lazy calls found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${lazyCalls} lazy call(s)`);
    }
    if (suspenseElements === 0) {
      console.log('✘ No Suspense components found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${suspenseElements} Suspense component(s)`);
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    const contentText = screen.getByTestId('lazy-content');
    if (contentText.textContent !== 'Lazy Loaded Content') {
      console.log('✘ Lazy content is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ Lazy content is rendered correctly');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Performance Optimization behavior is correct.');
    } else {
      console.log('\n❗ Performance Optimization behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
