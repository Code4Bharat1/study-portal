...
function codeVerify() {
  let allPassed = true;
  try {
    const ast = parser.parse(testCode || code, { sourceType: 'module', plugins: ['jsx'] });
    let testDescriptions = 0;

    traverse(ast, {
      CallExpression(path) {
        if (path.node.callee.name === 'describe' || path.node.callee.name === 'it') {
          testDescriptions++;
        }
      },
    });

    if (testCode && testDescriptions === 0) {
      console.log('✘ No test descriptions found in test file');
      allPassed = false;
    } else if (testCode) {
      console.log(`✔ Found ${testDescriptions} test description(s)`);
    } else {
...
async function functionalVerify() {
  ...
  try {
    ...
    render(<Component value="Test Value" />);
    const valueText = screen.getByTestId('value');
    if (valueText.textContent !== 'Test Value') {
      console.log('✘ Value is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ Value is rendered correctly');
    }

    render(<Component />);
    const defaultValueText = screen.getByTestId('value');
    if (defaultValueText.textContent !== 'Test Value') {
      console.log('✘ Default value is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ Default value is rendered correctly');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Testing Next.js Apps behavior is correct.');
    } else {
      console.log('\n❗ Testing Next.js Apps behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
