...
function codeVerify() {
  ...
  try {
    ...
    let useRouterCalls = 0;

    traverse(ast, {
      CallExpression(path) {
        if (path.node.callee.name === 'useRouter') {
          useRouterCalls++;
        }
      },
    });
    ...
    if (useRouterCalls === 0) {
      console.log('✘ No useRouter calls found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${useRouterCalls} useRouter call(s)`);
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    // Test English locale
    render(
      <RouterContext.Provider value={{ locale: 'en', push: jest.fn(), replace: jest.fn(), prefetch: jest.fn().mockResolvedValue(undefined) }}>
        <Component />
      </RouterContext.Provider>
    );
    const localeTextEn = screen.getByTestId('locale');
    if (localeTextEn.textContent !== 'Hello') {
      console.log('✘ English locale is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ English locale is rendered correctly');
    }

    // Test French locale
    render(
      <RouterContext.Provider value={{ locale: 'fr', push: jest.fn(), replace: jest.fn(), prefetch: jest.fn().mockResolvedValue(undefined) }}>
        <Component />
      </RouterContext.Provider>
    );
    const localeTextFr = screen.getByTestId('locale');
    if (localeTextFr.textContent !== 'Bonjour') {
      console.log('✘ French locale is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ French locale is rendered correctly');
    }

    if (allPassed) {
      console.log('\n🎉 Success! i18n behavior is correct.');
    } else {
      console.log('\n❗ i18n behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
