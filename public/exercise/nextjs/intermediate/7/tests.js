...
function codeVerify() {
  ...
  try {
    ...
    traverse(ast, {
      ExportNamedDeclaration(path) {
        if (
          ...
          ['getServerSideProps', 'getStaticProps'].includes(path.node.declaration.id.name)
        ) {
          serverSideFunctions++;
        }
      },
    ...
    if (serverSideFunctions > 0) {
      console.log('✘ Server-side functions found, incompatible with static export');
      allPassed = false;
    } else {
      console.log('✔ No server-side functions found, compatible with static export');
    }
...
async function functionalVerify() {
  ...
  try {
    ...
    if (content.textContent !== 'Static Export Content') {
      console.log('✘ Static export content is not rendered correctly');
      allPassed = false;
    } else {
      console.log('✔ Static export content is rendered correctly');
    }

    if (allPassed) {
      console.log('\n🎉 Success! Static Site Export behavior is correct.');
    } else {
      console.log('\n❗ Static Site Export behavior check failed. Please review your Next.js code.');
    }
    return allPassed;
  } catch (e) {
