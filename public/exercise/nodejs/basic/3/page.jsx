...
// File System Basics Verification
function codeVerify() {
  let allPassed = true;
  let ast;
  try {
  ...
  function traverse(node) {
    if (node.type === 'CallExpression' && node.callee.type === 'MemberExpression' && node.callee.object.name === 'fs' && ['readFileSync', 'writeFileSync'].includes(node.callee.property.name)) {
      fsMethods++;
    }
  ...
  if (fsMethods === 0) {
    console.log('✘ No fs.readFileSync or fs.writeFileSync calls found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${fsMethods} fs.readFileSync or fs.writeFileSync call(s)`);
  }

  let fsRequire = 0;
  traverse(ast);
  ast.body.forEach(node => {
    if (node.type === 'VariableDeclaration' && node.declarations.some(decl => decl.init && decl.init.type === 'CallExpression' && decl.init.callee.name === 'require' && decl.init.arguments[0].value === 'fs')) {
      fsRequire++;
    }
  });

  if (fsRequire === 0) {
    console.log("✘ No require('fs') found");
    allPassed = false;
  } else {
    console.log(`✔ Found ${fsRequire} require('fs') statement(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! File system basics are correct.');
  } else {
    console.log('\n❗ File system basics check failed. Please review your JavaScript.');
  }
  return allPassed;
}
