...
// Creating a Simple Server Verification
function codeVerify() {
  let allPassed = true;
  let ast;
  try {
  ...
  if (createServer === 0) {
    console.log('✘ No http.createServer calls found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${createServer} http.createServer call(s)`);
  }

  let httpRequire = 0;
  traverse(ast);
  ast.body.forEach(node => {
    if (node.type === 'VariableDeclaration' && node.declarations.some(decl => decl.init && decl.init.type === 'CallExpression' && decl.init.callee.name === 'require' && decl.init.arguments[0].value === 'http')) {
      httpRequire++;
    }
  });

  if (httpRequire === 0) {
    console.log("✘ No require('http') found");
    allPassed = false;
  } else {
    console.log(`✔ Found ${httpRequire} require('http') statement(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! Simple server creation is correct.');
  } else {
    console.log('\n❗ Simple server creation check failed. Please review your JavaScript.');
  }
  return allPassed;
}
