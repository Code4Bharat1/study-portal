...
function codeVerify() {
  ...
  if (eventEmitters === 0) {
    console.log('✘ No EventEmitter instances found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${eventEmitters} EventEmitter instance(s)`);
  }

  let eventMethods = 0;
  traverse(ast);
  ast.body.forEach(node => {
    if (node.type === 'ExpressionStatement' && node.expression.type === 'CallExpression' && node.expression.callee.type === 'MemberExpression' && ['on', 'emit'].includes(node.expression.callee.property.name)) {
      eventMethods++;
    }
  });

  if (eventMethods < 2) {
    console.log('✘ Fewer than 2 event methods (on, emit) found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${eventMethods} event method(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! Event emitters are correct.');
  } else {
    console.log('\n❗ Event emitters check failed. Please review your JavaScript.');
  }
  return allPassed;
}
