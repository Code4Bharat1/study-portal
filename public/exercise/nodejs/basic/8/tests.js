...
function codeVerify() {
  ...
  if (tryCatch === 0) {
    ...
    console.log(`✔ Found ${tryCatch} try-catch block(s)`);
  }

  let errorFirstCallbacks = 0;
  traverse(ast);
  ast.body.forEach(node => {
    if (node.type === 'FunctionDeclaration' && node.params.length > 0 && node.params[0].name === 'err') {
      errorFirstCallbacks++;
    }
  });

  if (errorFirstCallbacks === 0) {
    console.log('✘ No error-first callback functions found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${errorFirstCallbacks} error-first callback function(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! Basic error handling is correct.');
  } else {
    console.log('\n❗ Basic error handling check failed. Please review your JavaScript.');
  }
  return allPassed;
}
