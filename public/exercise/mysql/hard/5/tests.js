...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_advanced_5.json');
const PASS_FILE = path.join(__dirname, 'passed_advanced_5.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Assume stored procedure simulated by a special document with { procedure: 'myProc', executed: true }
    const doc = await new Promise((resolve, reject) => {
      coll.findOne({ procedure: 'myProc', executed: true }, (err, d) => {
        if (err) reject(new Error("Error querying 'advanced' collection."));
        else resolve(d);
      });
    });

    if (!doc) throw new Error("Stored procedure 'myProc' not executed (missing expected document).");
