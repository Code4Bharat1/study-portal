...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_advanced_3.json');
const PASS_FILE = path.join(__dirname, 'passed_advanced_3.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Assume partitioning simulated by documents with 'partition' field
    // Check that at least one document with partition: 'A' exists

    const doc = await new Promise((resolve, reject) => {
      coll.findOne({ partition: 'A' }, (err, d) => {
        if (err) reject(new Error("Error querying 'advanced' collection."));
        else resolve(d);
      });
    });

    if (!doc) throw new Error("No document with {partition:'A'} found. Partitioning expected.");
