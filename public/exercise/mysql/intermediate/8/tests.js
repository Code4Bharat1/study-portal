...
async function runTest() {
  ...
  try {
    ...
    // Check aggregated sales report
    const rows = await new Promise((resolve, reject) => {
      db.all(`
        SELECT product_id, SUM(quantity) as total_qty, SUM(price * quantity) as total_sales
        FROM sales
        GROUP BY product_id
      `, (err, rows) => {
        if (err) reject(err);
    ...
    if (!rows.length) {
      throw new Error("No aggregate sales data found.");
    }
