...
async function codeVerify() {
  ...
  try {
    ...
    let transforms = 0;
    root.walkDecls('transform', () => transforms++);
    if (transforms === 0) {
      console.log('✘ No transform properties found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${transforms} transform(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="transformed" data-testid="transformed">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const transformed = window.document.querySelector('[data-testid="transformed"]');
    const computedStyle = window.getComputedStyle(transformed);
    if (!computedStyle.transform.includes('matrix')) {
      console.log('✘ Transform not applied');
      allPassed = false;
    } else {
