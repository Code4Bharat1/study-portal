...
async function codeVerify() {
  ...
  try {
    ...
    let zIndexes = 0;
    root.walkDecls('z-index', () => zIndexes++);
    if (zIndexes === 0) {
      console.log('✘ No z-index properties found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${zIndexes} z-index(es)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="front" data-testid="front">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const front = window.document.querySelector('[data-testid="front"]');
    const computedStyle = window.getComputedStyle(front);
    if (computedStyle.zIndex !== '10') {
      console.log('✘ Z-index not applied');
      allPassed = false;
    } else {
