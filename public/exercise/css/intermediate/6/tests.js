...
async function codeVerify() {
  ...
  try {
    ...
    let filters = 0;
    root.walkDecls('filter', () => filters++);
    if (filters === 0) {
      console.log('✘ No filter properties found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${filters} filter(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="filtered" data-testid="filtered">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const filtered = window.document.querySelector('[data-testid="filtered"]');
    const computedStyle = window.getComputedStyle(filtered);
