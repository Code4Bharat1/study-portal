...
async function codeVerify() {
  ...
  try {
    ...
    let transitions = 0;
    root.walkDecls('transition', () => transitions++);
    if (transitions === 0) {
      console.log('✘ No transition properties found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${transitions} transition(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="transition" data-testid="transition">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const transition = window.document.querySelector('[data-testid="transition"]');
    const computedStyle = window.getComputedStyle(transition);
    if (!computedStyle.transition.includes('background-color')) {
      console.log('✘ Transition not applied');
      allPassed = false;
    } else {
