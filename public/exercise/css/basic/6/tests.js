...
async function codeVerify() {
  ...
  try {
    ...
    root.walkDecls('position', decl => {
      if (decl.value === 'absolute') positions++;
    });
    if (positions === 0) {
      console.log('✘ No absolute position found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${positions} absolute position(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="positioned" data-testid="positioned">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const positioned = window.document.querySelector('[data-testid="positioned"]');
    const computedStyle = window.getComputedStyle(positioned);
    if (computedStyle.position !== 'absolute' || computedStyle.top !== '10px' || computedStyle.left !== '20px') {
      console.log('✘ Positioning not applied');
      allPassed = false;
    } else {
      console.log('✔ Positioning applied');
    }
    return allPassed;
  } catch (e) {
