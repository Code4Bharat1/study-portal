...
async function codeVerify() {
  ...
  try {
    ...
    root.walkDecls('animation', decl => {
      if (decl.value.includes('cubic-bezier')) cubicBeziers++;
    });
    if (cubicBeziers === 0) {
      console.log('✘ No cubic-bezier animations found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${cubicBeziers} cubic-bezier animation(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="complex" data-testid="complex">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const complex = window.document.querySelector('[data-testid="complex"]');
    const computedStyle = window.getComputedStyle(complex);
    if (!computedStyle.animationTimingFunction.includes('cubic-bezier')) {
      console.log('✘ Complex animation not applied');
      allPassed = false;
    } else {
      console.log('✔ Complex animation applied');
    }
    return allPassed;
  } catch (e) {
