...
async function codeVerify() {
  ...
  try {
    ...
    let willChanges = 0;
    root.walkDecls('will-change', () => willChanges++);
    if (willChanges === 0) {
      console.log('✘ No will-change properties found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${willChanges} will-change(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="optimized" data-testid="optimized">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const optimized = window.document.querySelector('[data-testid="optimized"]');
    const computedStyle = window.getComputedStyle(optimized);
