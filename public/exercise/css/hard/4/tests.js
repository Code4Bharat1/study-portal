...
async function codeVerify() {
  ...
  try {
    ...
    let customPaints = 0;
    root.walkDecls(decl => {
      if (decl.value.includes('paint(')) customPaints++;
    });
    if (customPaints === 0) {
      console.log('✘ No paint API usage found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${customPaints} paint API usage(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="houdini" data-testid="houdini">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const houdini = window.document.querySelector('[data-testid="houdini"]');
    const computedStyle = window.getComputedStyle(houdini);
    if (!computedStyle.background.includes('var(--custom-paint)')) {
      console.log('✘ Houdini paint not applied');
      allPassed = false;
    } else {
      console.log('✔ Houdini paint applied');
    }
    return allPassed;
  } catch (e) {
