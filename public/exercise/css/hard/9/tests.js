...
async function codeVerify() {
  ...
  try {
    ...
    root.walkAtRules('container', () => containerQueries++);
    if (containerQueries === 0) {
      console.log('✘ No container queries found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${containerQueries} container query(ies)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    ...
    if (computedStyle.fontSize !== '20px') {
      console.log('✘ Container query not applied');
      allPassed = false;
    } else {
      console.log('✔ Container query applied');
    }
    return allPassed;
  } catch (e) {
