...
async function codeVerify() {
  ...
  try {
    ...
    let scrollSnaps = 0;
    root.walkDecls('scroll-snap-type', () => scrollSnaps++);
    if (scrollSnaps === 0) {
      console.log('✘ No scroll-snap-type found');
      allPassed = false;
    } else {
      console.log(`✔ Found ${scrollSnaps} scroll-snap-type(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    ...
    const carousel = window.document.querySelector('[data-testid="carousel"]');
    const computedStyle = window.getComputedStyle(carousel);
    if (!computedStyle.scrollSnapType.includes('x')) {
      console.log('✘ Scroll snap not applied');
      allPassed = false;
    } else {
      console.log('✔ Scroll snap applied');
    }
    return allPassed;
  } catch (e) {
