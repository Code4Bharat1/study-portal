...
async function codeVerify() {
  ...
  try {
    ...
    if (keyframes === 0) {
      ...
      console.log(`✔ Found ${keyframes} keyframe(s)`);
    }
    return allPassed;
  } catch (e) {
...
async function functionalVerify() {
  ...
  try {
    const dom = new JSDOM(`
      <div class="animated" data-testid="animated">Test</div>
    `);
    const { window } = dom;
    const style = window.document.createElement('style');
    style.textContent = css;
    window.document.head.appendChild(style);
    const animated = window.document.querySelector('[data-testid="animated"]');
    const computedStyle = window.getComputedStyle(animated);
