...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_advanced_9.json');
const PASS_FILE = path.join(__dirname, 'passed_advanced_9.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Assume audit logs stored as docs with 'action' and 'timestamp' fields
    const doc = await new Promise((resolve, reject) => {
      coll.findOne({ action: 'update', timestamp: { $exists: true } }, (err, d) => {
        if (err) reject(new Error("Error querying 'advanced' collection."));
        else resolve(d);
      });
    });

    if (!doc) throw new Error("No audit log document with action 'update' found.");
