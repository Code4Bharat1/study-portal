...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_advanced_8.json');
const PASS_FILE = path.join(__dirname, 'passed_advanced_8.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Assume user has restored backup by presence of a doc with { restored: true }
    const doc = await new Promise((resolve, reject) => {
      coll.findOne({ restored: true }, (err, d) => {
        if (err) reject(new Error("Error querying 'advanced' collection."));
        else resolve(d);
      });
    });

    if (!doc) throw new Error("Backup recovery document with {restored:true} not found.");
