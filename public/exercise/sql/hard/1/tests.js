...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_advanced_1.json');
const PASS_FILE = path.join(__dirname, 'passed_advanced_1.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Check if an index exists on 'field1' by ensuring query with sort returns docs
    // Since TingoDB doesn't expose indexes easily, we check if doc with field1: 'optimized' exists

    const doc = await new Promise((resolve, reject) => {
      coll.findOne({ field1: 'optimized' }, (err, d) => {
        if (err) reject(new Error("Error querying 'advanced' collection."));
        else resolve(d);
      });
    });

    if (!doc) throw new Error("Expected document with {field1:'optimized'} not found.");
