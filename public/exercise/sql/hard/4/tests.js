...
const ATTEMPT_FILE = path.join(__dirname, 'attempt_advanced_4.json');
const PASS_FILE = path.join(__dirname, 'passed_advanced_4.txt');
...
async function runTest() {
  ...
  try {
    ...
    // Assume user inserts docs with 'content' field containing text
    // Check if a document contains the keyword 'searchable'

    const doc = await new Promise((resolve, reject) => {
      coll.findOne({ content: /searchable/i }, (err, d) => {
        if (err) reject(new Error("Error querying 'advanced' collection."));
        else resolve(d);
      });
    });

    if (!doc) throw new Error("No document with 'searchable' in content found.");
