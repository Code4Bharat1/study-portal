...
# Structural verification for lambda functions
def code_verify():
    all_passed = True
    try:
        tree = ast.parse(code)
    ...
    if lambda_funcs == 0:
        ...
        print(f'✔ Found {lambda_funcs} lambda function(s)')
    if lambda_calls == 0:
        print('✘ No lambda function calls found')
        all_passed = False
    else:
        print(f'✔ Found {lambda_calls} lambda function call(s)')

    if all_passed:
        print('\n🎉 Success! Lambda functions implementation is correct.')
    else:
        print('\n❗ Lambda functions check failed. Please review your Python code.')
    return all_passed
