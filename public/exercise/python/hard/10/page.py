...
# Structural verification for CLI tools
def code_verify():
    all_passed = True
    try:
        tree = ast.parse(code)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.value.id == 'argparse' and node.func.attr == 'ArgumentParser':
            argparse_inits += 1
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.attr == 'add_argument' and isinstance(node.func.value, ast.Name):
            arg_adds += 1

    if argparse_inits == 0:
        print('✘ No ArgumentParser initializations found')
        all_passed = False
    else:
        print(f'✔ Found {argparse_inits} ArgumentParser initialization(s)')
    if arg_adds < 2:
        print('✘ Fewer than 2 add_argument calls found')
        all_passed = False
    else:
        print(f'✔ Found {arg_adds} add_argument call(s)')

    if all_passed:
        print('\n🎉 Success! CLI tools implementation is correct.')
    else:
        print('\n❗ CLI tools check failed. Please review your Python code.')
    return all_passed
