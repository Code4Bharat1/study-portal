...
# Structural verification for ctypes
def code_verify():
    all_passed = True
    try:
        tree = ast.parse(code)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and node.func.value.id == 'ctypes' and node.func.attr in ['cdll', 'windll']:
            ctypes_loads += 1
        if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute) and hasattr(node.func.value, 'id') and node.func.value.id.startswith('ctypes'):
            ctypes_calls += 1

    if ctypes_loads == 0:
        print('✘ No ctypes library loads found')
        all_passed = False
    else:
        print(f'✔ Found {ctypes_loads} ctypes library load(s)')
    if ctypes_calls == 0:
        print('✘ No ctypes function calls found')
        all_passed = False
    else:
        print(f'✔ Found {ctypes_calls} ctypes function call(s)')

    if all_passed:
        print('\n🎉 Success! C extensions implementation is correct.')
    else:
        print('\n❗ C extensions check failed. Please review your Python code.')
    return all_passed
