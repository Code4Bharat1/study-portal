...
# Structural verification for context managers
def code_verify():
    all_passed = True
    try:
        tree = ast.parse(code)
    ...
    contextlib_decorators = 0
    with_statements = 0
    for node in ast.walk(tree):
        if isinstance(node, ast.FunctionDef) and any(isinstance(dec, ast.Call) and isinstance(dec.func, ast.Attribute) and dec.func.value.id == 'contextlib' and dec.func.attr == 'contextmanager' for dec in node.decorator_list):
            contextlib_decorators += 1
        if isinstance(node, ast.With):
            with_statements += 1

    if contextlib_decorators == 0:
        print('✘ No contextlib.contextmanager decorators found')
        all_passed = False
    else:
        print(f'✔ Found {contextlib_decorators} contextlib.contextmanager decorator(s)')
    if with_statements == 0:
        print('✘ No with statements found')
        all_passed = False
    else:
        print(f'✔ Found {with_statements} with statement(s)')

    if all_passed:
        print('\n🎉 Success! Context managers implementation is correct.')
    else:
        print('\n❗ Context managers check failed. Please review your Python code.')
    return all_passed
