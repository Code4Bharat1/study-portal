...
# Structural verification for file I/O
def code_verify():
    all_passed = True
    try:
        tree = ast.parse(code)
    ...
    for node in ast.walk(tree):
        if isinstance(node, ast.With) and isinstance(node.items[0].context_expr, ast.Call) and isinstance(node.items[0].context_expr.func, ast.Name) and node.items[0].context_expr.func.id == 'open':
            if 'r' in node.items[0].context_expr.args[1].s if len(node.items[0].context_expr.args) > 1 else '':
                file_reads += 1
            if 'w' in node.items[0].context_expr.args[1].s if len(node.items[0].context_expr.args) > 1 else '':
                file_writes += 1

    if file_reads == 0:
        print('✘ No file read operations found')
        all_passed = False
    else:
        print(f'✔ Found {file_reads} file read operation(s)')
    if file_writes == 0:
        print('✘ No file write operations found')
        all_passed = False
    else:
        print(f'✔ Found {file_writes} file write operation(s)')

    if all_passed:
        print('\n🎉 Success! File I/O implementation is correct.')
    else:
        print('\n❗ File I/O check failed. Please review your Python code.')
    return all_passed
