...
# Structural verification for OOP
def code_verify():
    all_passed = True
    try:
        tree = ast.parse(code)
    ...
    if classes == 0:
        ...
        print(f'✔ Found {classes} class definition(s)')
    if inherited_classes == 0:
        print('✘ No inherited classes found')
        all_passed = False
    else:
        print(f'✔ Found {inherited_classes} inherited class(es)')
    if methods == 0:
        print('✘ No class methods found')
        all_passed = False
    else:
        print(f'✔ Found {methods} class method(s)')

    if all_passed:
        print('\n🎉 Success! OOP implementation is correct.')
    else:
        print('\n❗ OOP check failed. Please review your Python code.')
    return all_passed
