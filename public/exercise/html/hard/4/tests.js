...
function codeVerify($) {
  ...
  const nav = $('body nav');
  if (nav.length > 0) {
    console.log(`✔ Found ${nav.length} <nav> tag(s)`);
    let navValid = true;
    nav.each((index, elem) => {
      const links = $(elem).find('a[href]');
      if (links.length < 3) {
        console.log(`✘ Navigation ${index + 1} has fewer than 3 <a> elements`);
        navValid = false;
        allPassed = false;
      } else {
        console.log(`✔ Navigation ${index + 1} has ${links.length} <a> elements`);
      }
      const ariaLabel = $(elem).attr('aria-label');
      if (!ariaLabel || ariaLabel.trim() === '') {
        console.log(`✘ Navigation ${index + 1} missing or empty aria-label`);
        navValid = false;
        allPassed = false;
      } else {
        console.log(`✔ Navigation ${index + 1} has aria-label: "${ariaLabel}"`);
      }
      const toggleButton = $(elem).find('button[aria-controls]');
      if (toggleButton.length === 0) {
        console.log(`✘ Navigation ${index + 1} missing button with aria-controls for off-canvas`);
        navValid = false;
        allPassed = false;
      } else {
        console.log(`✔ Navigation ${index + 1} has button with aria-controls`);
      }
    });
    if (navValid) {
      console.log('✔ All navigation elements are valid');
    }
  } else {
    console.log('✘ At least one <nav> is required for off-canvas menu');
    allPassed = false;
  }

  if (allPassed) {
    console.log('\n🎉 Success! HTML boilerplate and off-canvas navigation are correct.');
  } else {
    console.log('\n❗ Boilerplate or off-canvas navigation check failed. Please review your HTML.');
  }
