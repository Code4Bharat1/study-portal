...
// Advanced Metadata for SEO Verification
function codeVerify($) {
  let allPassed = true;
  ...
  if (metaDescription.length > 0 && metaDescription.attr('content').trim()) {
    console.log('✔ <meta name="description"> is present with content');
  } else {
    console.log('✘ <meta name="description"> is missing or empty');
    allPassed = false;
  }

  const ogTitle = $('head meta[property="og:title"]');
  if (ogTitle.length > 0 && ogTitle.attr('content').trim()) {
    console.log('✔ <meta property="og:title"> is present with content');
  } else {
    console.log('✘ <meta property="og:title"> is missing or empty');
    allPassed = false;
  }

  const ogDescription = $('head meta[property="og:description"]');
  if (ogDescription.length > 0 && ogDescription.attr('content').trim()) {
    console.log('✔ <meta property="og:description"> is present with content');
  } else {
    console.log('✘ <meta property="og:description"> is missing or empty');
    allPassed = false;
  }

  const ogImage = $('head meta[property="og:image"]');
  if (ogImage.length > 0 && ogImage.attr('content').trim()) {
    console.log('✔ <meta property="og:image"> is present with content');
  } else {
    console.log('✘ <meta property="og:image"> is missing or empty');
    allPassed = false;
  }

  const twitterCard = $('head meta[name="twitter:card"]');
  if (twitterCard.length > 0 && twitterCard.attr('content').trim()) {
    console.log('✔ <meta name="twitter:card"> is present with content');
  } else {
    console.log('✘ <meta name="twitter:card"> is missing or empty');
    allPassed = false;
  }

  if (allPassed) {
    console.log('\n🎉 Success! HTML boilerplate and SEO metadata are correct.');
  } else {
    console.log('\n❗ Boilerplate or SEO metadata check failed. Please review your HTML.');
  }
