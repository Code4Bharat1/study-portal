...
// Microdata and Schema.org Verification
function codeVerify($) {
  let allPassed = true;
  ...
  // Microdata and Schema.org Checks
  const itemscopes = $('[itemscope]');
  if (itemscopes.length > 0) {
    console.log(`✔ Found ${itemscopes.length} element(s) with itemscope`);
    let validMicrodata = true;
    itemscopes.each((index, elem) => {
      const itemtype = $(elem).attr('itemtype');
      if (!itemtype || !itemtype.includes('schema.org')) {
        console.log(`✘ Element ${index + 1} with itemscope missing or invalid itemtype (must include schema.org)`);
        validMicrodata = false;
        allPassed = false;
      } else {
        console.log(`✔ Element ${index + 1} has valid itemtype: "${itemtype}"`);
      }
      const itemprops = $(elem).find('[itemprop]');
      if (itemprops.length === 0) {
        console.log(`✘ Element ${index + 1} with itemscope has no itemprop attributes`);
        validMicrodata = false;
        allPassed = false;
      } else {
        console.log(`✔ Element ${index + 1} has ${itemprops.length} itemprop attributes`);
        itemprops.each((i, prop) => {
          const propName = $(prop).attr('itemprop');
          if (!propName || propName.trim() === '') {
            console.log(`✘ Itemprop ${i + 1} in element ${index + 1} is empty`);
            validMicrodata = false;
            allPassed = false;
          } else {
            console.log(`✔ Itemprop ${i + 1} in element ${index + 1}: "${propName}"`);
          }
        });
      }
    });
    if (validMicrodata) {
      console.log('✔ All microdata structures are valid');
    }
  } else {
    console.log('✘ At least one element with itemscope is required');
    allPassed = false;
  }

  if (allPassed) {
    console.log('\n🎉 Success! HTML boilerplate and microdata are correct.');
  } else {
    console.log('\n❗ Boilerplate or microdata check failed. Please review your HTML.');
  }
