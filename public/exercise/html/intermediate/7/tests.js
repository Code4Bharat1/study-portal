...
// Boilerplate and Iframe Verification
function codeVerify($) {
  let allPassed = true;
  ...
  if (iframes.length > 0) {
    ...
    iframes.each((index, iframe) => {
      ...
      if (!src || src.trim() === '') {
        console.log(`✘ Iframe ${index + 1} missing src`);
        allIframesValid = false;
        allPassed = false;
      } else {
        const urlPattern = /^(https?:\/\/|\/|[a-zA-Z0-9_-]+\.(html|php))/i;
        if (!urlPattern.test(src)) {
          console.log(`✘ Iframe ${index + 1} has invalid src: "${src}"`);
          allIframesValid = false;
          allPassed = false;
        } else {
          console.log(`✔ Iframe ${index + 1} has valid src`);
        }
      }
      const title = $(iframe).attr('title');
      if (!title || title.trim() === '') {
        console.log(`✘ Iframe ${index + 1} missing or empty title`);
        allIframesValid = false;
        allPassed = false;
      } else {
        console.log(`✔ Iframe ${index + 1} has title: "${title}"`);
      }
    });
    if (allIframesValid) {
      console.log('✔ All <iframe> tags are valid');
    }
  } else {
    console.log('✘ At least one <iframe> is required');
    allPassed = false;
  }

  if (allPassed) {
    console.log('\n🎉 Success! HTML boilerplate and iframes are correct.');
  } else {
    console.log('\n❗ Boilerplate or iframe check failed. Please review your HTML.');
  }
