...
// Boilerplate and Figure/Caption Verification
function codeVerify($) {
  let allPassed = true;
  ...
  const figures = $('body figure');
  if (figures.length > 0) {
    console.log(`✔ Found ${figures.length} <figure> tag(s)`);
    let allFiguresValid = true;
    figures.each((index, figure) => {
      const img = $(figure).find('img');
      const code = $(figure).find('code');
      if (img.length === 0 && code.length === 0) {
        console.log(`✘ Figure ${index + 1} missing <img> or <code>`);
        allFiguresValid = false;
        allPassed = false;
      } else {
        if (img.length > 0) {
          console.log(`✔ Figure ${index + 1} has <img>`);
          img.each((i, imgElem) => {
            const src = $(imgElem).attr('src');
            if (!src || src.trim() === '') {
              console.log(`✘ Figure ${index + 1} image ${i + 1} missing src`);
              allFiguresValid = false;
              allPassed = false;
            } else {
              console.log(`✔ Figure ${index + 1} image ${i + 1} has src`);
            }
          });
        }
        if (code.length > 0) {
          console.log(`✔ Figure ${index + 1} has <code>`);
          code.each((i, codeElem) => {
            const text = $(codeElem).text().trim();
            if (!text) {
              console.log(`✘ Figure ${index + 1} code ${i + 1} is empty`);
              allFiguresValid = false;
              allPassed = false;
            } else {
              console.log(`✔ Figure ${index + 1} code ${i + 1} has content`);
            }
          });
        }
      }
      ...
      if (figcaption.length === 0) {
        console.log(`✘ Figure ${index + 1} missing <figcaption>`);
        allFiguresValid = false;
        allPassed = false;
      } else {
        console.log(`✔ Figure ${index + 1} has <figcaption>`);
        const captionText = figcaption.text().trim();
        if (!captionText) {
          console.log(`✘ Figure ${index + 1} <figcaption> is empty`);
          allFiguresValid = false;
          allPassed = false;
        } else {
          console.log(`✔ Figure ${index + 1} <figcaption> has text`);
        }
      }
    });
    if (allFiguresValid) {
      console.log('✔ All <figure> tags are valid');
    }
  } else {
    console.log('✘ At least one <figure> is required');
    allPassed = false;
  }

  if (allPassed) {
    console.log('\n🎉 Success! HTML boilerplate and figure/caption elements are correct.');
  } else {
    console.log('\n❗ Boilerplate or figure/caption check failed. Please review your HTML.');
  }
