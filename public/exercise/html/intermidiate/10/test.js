...
// Boilerplate and Form with Validation Verification
function codeVerify($) {
  let allPassed = true;
  ...
  if (forms.length > 0) {
    ...
    forms.each((index, form) => {
      ...
      if (fieldset.length === 0) {
        console.log(`✘ Form ${index + 1} missing <fieldset>`);
        allFormsValid = false;
        allPassed = false;
      } else {
        console.log(`✔ Form ${index + 1} has <fieldset>`);
        fieldset.each((i, fs) => {
          const legend = $(fs).find('legend');
          if (legend.length === 0) {
            console.log(`✘ Form ${index + 1} fieldset ${i + 1} missing <legend>`);
            allFormsValid = false;
            allPassed = false;
          } else {
            console.log(`✔ Form ${index + 1} fieldset ${i + 1} has <legend>`);
            const legendText = legend.text().trim();
            if (!legendText) {
              console.log(`✘ Form ${index + 1} fieldset ${i + 1} <legend> is empty`);
              allFormsValid = false;
              allPassed = false;
            } else {
              console.log(`✔ Form ${index + 1} fieldset ${i + 1} <legend> has text`);
            }
          }
        });
      }
      const validInput = $(form).find('input[required][pattern][minlength]');
      if (validInput.length === 0) {
        console.log(`✘ Form ${index + 1} missing <input> with required, pattern, and minlength`);
        allFormsValid = false;
        allPassed = false;
      } else {
        console.log(`✔ Form ${index + 1} has <input> with required, pattern, and minlength`);
        validInput.each((i, input) => {
          const pattern = $(input).attr('pattern');
          const minlength = $(input).attr('minlength');
          if (!pattern || pattern.trim() === '') {
            console.log(`✘ Form ${index + 1} input ${i + 1} has empty pattern`);
            allFormsValid = false;
            allPassed = false;
          } else {
            console.log(`✔ Form ${index + 1} input ${i + 1} has pattern`);
          }
          if (!minlength || isNaN(minlength) || parseInt(minlength) <= 0) {
            console.log(`✘ Form ${index + 1} input ${i + 1} has invalid minlength`);
            allFormsValid = false;
            allPassed = false;
          } else {
            console.log(`✔ Form ${index + 1} input ${i + 1} has valid minlength`);
          }
        });
      }
  ...
  if (allPassed) {
    console.log('\n🎉 Success! HTML boilerplate and form with validation are correct.');
  } else {
    console.log('\n❗ Boilerplate or form validation check failed. Please review your HTML.');
  }
