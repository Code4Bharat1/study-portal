...
// Loops and Iteration Verification
function codeVerify() {
  let allPassed = true;
  let ast;
  try {
  ...
  if (loops === 0) {
    console.log('✘ No loops (for, while, or do-while) found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${loops} loop(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! Loops and iteration are correct.');
  } else {
    console.log('\n❗ Loops or iteration check failed. Please review your JavaScript.');
  }
  return allPassed;
}
