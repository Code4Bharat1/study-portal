...
function codeVerify() {
  ...
  function traverse(node) {
    if (node.type === 'BinaryExpression' && operators.includes(node.operator)) {
      foundOperators.add(node.operator);
    }
  ...
  operators.forEach(op => {
    if (foundOperators.has(op)) {
      console.log(`✔ Found ${op} operator`);
    } else {
      console.log(`✘ Missing ${op} operator`);
      allPassed = false;
    }
  });

  if (allPassed) {
    console.log('\n🎉 Success! Arithmetic operations are correct.');
  } else {
    console.log('\n❗ Arithmetic operations check failed. Please review your JavaScript.');
  }
  return allPassed;
}
