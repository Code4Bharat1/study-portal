...
// Promises and Async/Await Verification
function codeVerify() {
  let allPassed = true;
  let ast;
  try {
  ...
  if (promises === 0) {
    console.log('✘ No Promise instances found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${promises} Promise instance(s)`);
  }
  if (asyncFunctions === 0) {
  ...
  if (allPassed) {
    console.log('\n🎉 Success! Promises and async/await are correct.');
  } else {
    console.log('\n❗ Promises or async/await check failed. Please review your JavaScript.');
  }
  return allPassed;
}
