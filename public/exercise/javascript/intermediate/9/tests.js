...
// Error Handling with Custom Errors Verification
function codeVerify() {
  let allPassed = true;
  let ast;
  try {
  ...
  ast.body.forEach(node => {
    if (node.type === 'ClassDeclaration' && node.superClass && node.superClass.name === 'Error') {
      customErrors++;
    }
  });
  if (customErrors === 0) {
    console.log('✘ No custom Error subclasses found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${customErrors} custom Error subclass(es)`);
  }
  ...
  if (throws === 0) {
    ...
    console.log(`✔ Found ${throws} throw statement(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! Custom error handling is correct.');
  } else {
    console.log('\n❗ Custom error handling check failed. Please review your JavaScript.');
  }
  return allPassed;
}
