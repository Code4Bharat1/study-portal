...
function codeVerify() {
  ...
  function traverse(node) {
    if (node.type === 'CallExpression' && node.callee.type === 'MemberExpression' && node.callee.property.name === 'addEventListener') {
      ...
      if (callback.type === 'FunctionExpression' && callback.body.body.some(child => child.type === 'IfStatement' && child.test.type === 'CallExpression' && child.test.callee.type === 'MemberExpression' && child.test.callee.property.name === 'matches')) {
        eventListeners++;
      }
    }
  ...
  if (eventListeners === 0) {
    console.log('✘ No event delegation (addEventListener with matches) found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${eventListeners} event delegation instance(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! Event delegation is correct.');
  } else {
    console.log('\n❗ Event delegation check failed. Please review your JavaScript.');
  }
  return allPassed;
}
