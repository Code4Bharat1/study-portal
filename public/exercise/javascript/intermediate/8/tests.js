...
function codeVerify() {
  ...
  if (regexMethods === 0) {
    console.log('✘ No regular expression methods (test or exec) found');
    allPassed = false;
  } else {
    console.log(`✔ Found ${regexMethods} regular expression method(s)`);
  }

  if (allPassed) {
    console.log('\n🎉 Success! Regular expressions are correct.');
  } else {
    console.log('\n❗ Regular expressions check failed. Please review your JavaScript.');
  }
  return allPassed;
}
