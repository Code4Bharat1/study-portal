...
export default function MongoDBProjects() {
  ...
  const [code, setCode] = useState(`db.users.insertOne({ name: "John Doe", email: "john@example.com", age: 25 });`)
  ...
  const fetchProjects = async () => {
    try {
      const token = localStorage.getItem('token')
      const response = await fetch('http://localhost:5000/api/projects?category=mongodb', {
  ...
  const fetchScores = async () => {
    try {
      ...
      const token = localStorage.getItem('token')
      const response = await fetch(`http://localhost:5000/api/scores?userId=${userId}`, {
        headers: { 'Authorization': `Bearer ${token}` }
      })
      if (!response.ok) {
        if (response.status === 403) {
    ...
    <main className="min-h-screen bg-green-100 px-4 py-10">
      <div className="max-w-6xl mx-auto">
        <nav className="flex justify-center space-x-4 mb-8">
          ...
          <Link href="/mongodb-projects" className="text-green-600 hover:underline">MongoDB</Link>
          <Link href="/express-projects" className="text-blue-600 hover:underline">Express</Link>
          <Link href="/node-projects" className="text-purple-600 hover:underline">Node.js</Link>
          <Link href="/react-projects" className="text-red-600 hover:underline">React</Link>
          <button onClick={handleLogout} className="text-gray-600 hover:underline">Logout</button>
        </nav>
        <h1 className="text-5xl font-bold text-black mb-8 text-center">MongoDB Projects</h1>
          ...
          <div className="bg-white rounded-lg shadow-md p-6">
            ...
            <ul className="list-disc pl-5 text-black">
              {projects.basic.map((project) => (
                <li key={project._id}>
                  <Link href={`/mongodb-projects/${project.slug}`} className="text-green-600 hover:underline">
                    {project.title}
                  </Link>
                </li>
              ))}
        ...
        <div className="bg-white rounded-lg shadow-md p-6 mb-12">
          <h2 className="text-2xl font-semibold text-black mb-4">Practice MongoDB Code</h2>
          <p className="text-black mb-4">Write and test your MongoDB code below. Click "Run Code" to execute it.</p>
          <textarea
        ...
        <div className="bg-white rounded-lg shadow-md p-6">
          <h2 className="text-2xl font-semibold text-black mb-4">Your Scores</h2>
          {scores.length > 0 ? (
            <ul className="list-disc pl-5 text-black">
              {scores.map((score) => (
                <li key={score._id}>
                  Score: {score.score} - {new Date(score.timestamp).toLocaleString()}
                </li>
              ))}
            </ul>
          ) : (
            <p className="text-black">No scores yet. Run some code to earn scores!</p>
          )}
        </div>
      </div>
    </main>
