          ...
          <ul className="absolute left-1/2 transform -translate-x-1/2 hidden md:flex items-center gap-8 text-black text-sm font-semibold w-max">
            {navbarItems.map((item, index) => (
              <li key={index}>
                <Link
                  href={item.link}
                  className={`hover:text-blue-500 hover:border-b-2 hover:border-black transition-all duration-300 cursor-pointer ${
                    pathname === item.link
      ...
      <div className="fixed top-[60px] left-0 w-full backdrop-blur-md bg-gray-200 border-t border-b border-white/30 flex items-center px-4 py-1 gap-6 shadow-md z-40 overflow-x-auto whitespace-nowrap">
        {menuItems.map((item, index) => (
          <Link
            href={item.link}
            key={index}
            className={`text-gray-800 hover:text-blue-600 hover:border-b-2 hover:border-black text-sm font-semibold transition-all duration-300 border-2 border-transparent px-4 py-2 ${
              pathname === item.link ? 'border-blue-600' : ''
            }`}
          >
            {item.name}
          </Link>
