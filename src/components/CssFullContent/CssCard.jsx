    ...
    testCases: [
      {
        ...
        points: 25, // Adjusted for base score (total 70)
      },
      {
    ...
    testCases: [
      {
        ...
        points: 20, // Adjusted for base score (total 70)
      },
      {
    ...
    testCases: [
      {
        ...
        points: 15, // Adjusted for base score (total 70)
      },
      {
...
export default function CssSandboxPage() {
  ...
  const evaluateCode = () => {
    ...
    // Base Score: Sum of test case points (max 70)
    let baseScore = 0;
    ...
    // Time Penalty: Deduct up to 10 points based on time used
    const timeLimit = challenge.timeLimit;
    const timeUsed = timeLimit - timeLeft;
    const timePercentage = (timeUsed / timeLimit) * 100;
    const timeDeduction = (timePercentage / 100) * 10; // Max deduction of 10 points
    const timeScore = Math.max(0, 10 - timeDeduction);

    // Error Penalty: Deduct 2 points per error, capped at 10 points
    const errorDeduction = Math.min(errorCount * 2, 10); // 2 points per error, max 10
    const errorScore = 10 - errorDeduction;

    // Efficiency Bonus: Bonus points based on number of runs
    let efficiencyBonus = 0;
    if (runCount + 1 <= 2) {
      efficiencyBonus = 10; // 1-2 runs: 10 points
    } else if (runCount + 1 <= 5) {
      efficiencyBonus = 5; // 3-5 runs: 5 points
    } // 6+ runs: 0 points
