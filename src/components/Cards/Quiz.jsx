...
const quizzes = [
  { name: 'MongoDB', link: '/quiz/mongodb', color: 'from-green-500 to-emerald-600', icon: <SiMongodb size={40} /> },
  { name: 'Express', link: '/quiz/express', color: 'from-blue-500 to-indigo-600', icon: <SiExpress size={40} /> },
  { name: 'React', link: '/quiz/react', color: 'from-cyan-500 to-teal-600', icon: <SiReact size={40} /> },
  { name: 'Node.js', link: '/quiz/nodejs', color: 'from-lime-500 to-green-600', icon: <SiNodedotjs size={40} /> },
...
export default function QuizSelection() {
  return (
    <div className="relative min-h-screen flex flex-col items-center justify-start p-6 overflow-hidden bg-white mt-30">
      {/* Heading */}
      <motion.h1
        initial={{ opacity: 0, y: 20 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ duration: 0.6, ease: 'easeOut' }}
        className="relative z-10 text-5xl font-bold text-transparent bg-clip-text bg-gradient-to-r from-blue-600 to-indigo-700 text-center mt-8 mb-16 drop-shadow-md tracking-tight"
      >
        Choose Your Quiz
      </motion.h1>
