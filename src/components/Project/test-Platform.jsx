...
export default function ProjectPlatform({
  ...
  const handleSubmit = async () => {
    try {
      ...
      if ("results.tests" in fsSnap && fsSnap["results.tests"]) {
        setUrl(`${(title.replace(".", "")).toLowerCase()}/${selectedLevel.toLowerCase()}/${menuItems.find(item => item.label === selected)+1}`)
  ...
  const checkPreviousExerciseAttempted = async () => {
    ...
    const vm = await sdk.connect(container);
    console.log("Connected VM checking:", vm);
  ...
  const extendedMenu = useMemo(() => {
    return menuItems.map((item) => ({
      ...
      onClick: async (e) => {
        ...
        console.log("prev", previousAttempt);
        if (!previousAttempt){
          item.onClick?.(e);
          setShowingInstructions(true);
          return 1;
