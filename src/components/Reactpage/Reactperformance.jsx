...
{`const MyComponent = React.memo(function MyComponent(props) {
  return <div>{props.value}</div>;
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">Why use React.memo?</summary>
          <p className="mt-2">
            <code>React.memo</code> helps avoid re-rendering components when their props haven’t changed.
            This is especially useful when the component is complex or renders large datasets.
          </p>
        </details>
...
{`const handleClick = useCallback(() => {
  console.log("Clicked");
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">When should you use useCallback?</summary>
          <p className="mt-2">
            Use <code>useCallback</code> when passing functions to child components to prevent unnecessary re-renders due to new function references.
          </p>
        </details>
...
{`const expensiveValue = useMemo(() => {
  return computeExpensiveValue(input);
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">Why use useMemo?</summary>
          <p className="mt-2">
            It optimizes performance by caching heavy computations and reusing them unless dependencies change.
          </p>
        </details>
  ...
  return (
    <Suspense fallback={<div>Loading...</div>}>
      <LazyComponent />
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">How does lazy loading help?</summary>
          <p className="mt-2">
            It reduces the initial bundle size by loading components only when they’re needed, which makes your app load faster.
          </p>
        </details>
...
>
  {({ index, style }) => <div style={style}>Item {index}</div>}
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">Why use windowing?</summary>
          <p className="mt-2">
            Rendering only visible items improves performance and memory usage for long lists or tables.
          </p>
        </details>
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">Why avoid inline functions?</summary>
          <p className="mt-2">
            Inline functions in JSX create new function instances on every render, which can trigger unnecessary re-renders in child components.
          </p>
        </details>
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">Why is prop drilling bad for performance?</summary>
          <p className="mt-2">
            It leads to complex component hierarchies and causes re-renders even if the intermediate components don’t use the data.
          </p>
        </details>
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">How do these tools help?</summary>
          <p className="mt-2">
            They help you inspect component re-renders, analyze timing, and identify memory leaks or slow operations.
          </p>
        </details>
        ...
        <details className="mb-6 text-gray-700">
          <summary className="cursor-pointer font-medium">Why production builds matter?</summary>
          <p className="mt-2">
            Production builds are optimized, minified, and strip out helpful but heavy development checks to improve performance.
          </p>
        </details>
