...
export default function ExpressSandbox({ 
  filesObj, 
  fileToOpen, 
  onLoad, 
  hideExplorer = true 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'app.js');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('express-theme', {
      base: 'vs',
      inherit: true,
      rules: [
    ...
    monaco.editor.setTheme('express-theme');
  };
  ...
  const handleRunTests = async () => {
    ...
    setIsRunning(true);
    setOutput('🚀 Running Express tests...\n');
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No Express test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ Express test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 Express Test Results:\n';
        ...
        setOutput('❌ Express test system not available - window.exerciseTest not found');
        console.error('Express test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('Express test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/3 border-l flex flex-col">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="text-sm font-medium">Express Output</h3>
          </div>
          <div 
            ref={outputRef}
