...
export default function ReactSandbox({ 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'App.jsx');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('react-theme', {
      base: 'vs',
      inherit: true,
      rules: [
    ...
    monaco.editor.setTheme('react-theme');
  };
  ...
  const handleRunTests = async () => {
    ...
    setIsRunning(true);
    setOutput('⚛️ Running React tests...\n');
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No React test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ React test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 React Test Results:\n';
        ...
        setOutput('❌ React test system not available - window.exerciseTest not found');
        console.error('React test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('React test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/3 border-l flex flex-col">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="text-sm font-medium">React Output</h3>
          </div>
          <div 
            ref={outputRef}
