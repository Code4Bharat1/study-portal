...
export default function NodeJSSandbox({ 
  filesObj, 
  fileToOpen, 
  onLoad, 
  hideExplorer = true 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'server.js');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('nodejs-theme', {
      base: 'vs',
      inherit: true,
      rules: [],
      colors: {
    ...
    monaco.editor.setTheme('nodejs-theme');
  ...
  const executeNodeJS = (code) => {
    return new Promise((resolve) => {
      ...
      try {
        const executeCode = new Function('console', 'require', 'module', 'exports', code);
        const mockRequire = (module) => {
          if (module === 'fs') return { readFileSync: () => 'mock file content', writeFileSync: () => {} };
          if (module === 'path') return { join: (...args) => args.join('/') };
          if (module === 'http') return { createServer: () => ({ listen: () => {} }) };
          return {};
        };
        const mockModule = { exports: {} };
        executeCode(mockConsole, mockRequire, mockModule, mockModule.exports);
        
        resolve({
          output: logs.join('\n') || 'Node.js code executed (browser simulation)',
          errors: errors.join('\n'),
  ...
  const handleRunCode = async () => {
    ...
    setIsRunning(true);
    setOutput('🟢 Running Node.js code...\n');
    
    try {
      const code = files[activeFile];
      const result = await executeNodeJS(code);
  ...
  const handleRunTests = async () => {
    ...
    setIsRunning(true);
    setOutput('🧪 Running Node.js tests...\n');
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No Node.js test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ Node.js test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 Node.js Test Results:\n';
        ...
        setOutput('❌ Node.js test system not available - window.exerciseTest not found');
        console.error('Node.js test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('Node.js test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/3 border-l flex flex-col">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="text-sm font-medium">Node.js Output</h3>
          </div>
          <div 
            ref={outputRef}
