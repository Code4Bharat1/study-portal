...
export default function MySQLSandbox({ 
  filesObj, 
  fileToOpen, 
  onLoad, 
  hideExplorer = true 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'query.sql');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('mysql-theme', {
      base: 'vs',
      inherit: true,
      rules: [
    ...
    monaco.editor.setTheme('mysql-theme');
  };
  ...
  const handleRunTests = async () => {
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No MySQL test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ MySQL test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 MySQL Test Results:\n';
        ...
        setOutput('❌ MySQL test system not available - window.exerciseTest not found');
        console.error('MySQL test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('MySQL test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/3 border-l flex flex-col">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="text-sm font-medium">MySQL Output</h3>
          </div>
          <div 
            ref={outputRef}
