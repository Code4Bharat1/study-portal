...
export default function PHPSandbox({ 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'index.php');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('php-theme', {
      base: 'vs',
      inherit: true,
      rules: [
        { token: 'keyword', foreground: '#0066cc' },
        { token: 'string', foreground: '#cc6600' },
        { token: 'variable', foreground: '#990099' }
      ],
      colors: {
    ...
    monaco.editor.setTheme('php-theme');
  };
  ...
  const handleRunTests = async () => {
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No PHP test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ PHP test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 PHP Test Results:\n';
        ...
        setOutput('❌ PHP test system not available - window.exerciseTest not found');
        console.error('PHP test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('PHP test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/3 border-l flex flex-col">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="text-sm font-medium">PHP Output</h3>
          </div>
          <div 
            ref={outputRef}
