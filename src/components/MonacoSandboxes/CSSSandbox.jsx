...
export default function CSSSandbox({ 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'styles.css');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('css-theme', {
      base: 'vs',
      inherit: true,
      rules: [
        { token: 'selector', foreground: '#0066cc' },
        { token: 'property', foreground: '#cc6600' },
        { token: 'value', foreground: '#990099' }
      ],
      colors: {
    ...
    monaco.editor.setTheme('css-theme');
  };
  ...
  const handleRunTests = async () => {
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No CSS test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ CSS test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 CSS Test Results:\n';
        ...
        setOutput('❌ CSS test system not available - window.exerciseTest not found');
        console.error('CSS test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
