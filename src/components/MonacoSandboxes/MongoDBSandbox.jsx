...
export default function MongoDBSandbox({ 
  filesObj, 
  fileToOpen, 
  onLoad, 
  hideExplorer = true 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'query.js');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('mongodb-theme', {
      base: 'vs',
      inherit: true,
      rules: [
        { token: 'keyword', foreground: '#0066cc' },
        { token: 'string', foreground: '#cc6600' },
        { token: 'identifier', foreground: '#990099' }
      ],
      colors: {
    ...
    monaco.editor.setTheme('mongodb-theme');
  };
  ...
  const handleRunTests = async () => {
    ...
    setIsRunning(true);
    setOutput('🍃 Running MongoDB tests...\n');
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No MongoDB test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ MongoDB test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 MongoDB Test Results:\n';
        ...
        setOutput('❌ MongoDB test system not available - window.exerciseTest not found');
        console.error('MongoDB test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('MongoDB test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/3 border-l flex flex-col">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="text-sm font-medium">MongoDB Output</h3>
          </div>
          <div 
            ref={outputRef}
