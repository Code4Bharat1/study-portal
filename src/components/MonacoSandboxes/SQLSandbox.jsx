...
export default function SQLSandbox({ 
  ...
  const handleRunTests = async () => {
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No SQL test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ SQL test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        let outputText = '📊 SQL Test Results:\n';
        ...
        setOutput('❌ SQL test system not available - window.exerciseTest not found');
        console.error('SQL test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('SQL test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
      ...
      <div className="flex flex-1">
        ...
        <div className="w-1/3 border-l flex flex-col">
          <div className="bg-gray-50 px-4 py-2 border-b">
            <h3 className="text-sm font-medium">SQL Output</h3>
          </div>
          <div 
            ref={outputRef}
