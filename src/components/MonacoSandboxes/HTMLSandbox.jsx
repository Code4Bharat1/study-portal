...
export default function HTMLSandbox({ 
  ...
  const [activeFile, setActiveFile] = useState(fileToOpen || 'index.html');
  ...
  const handleEditorDidMount = (editor, monaco) => {
    ...
    monaco.editor.defineTheme('html-theme', {
      base: 'vs',
      inherit: true,
      rules: [
        { token: 'tag', foreground: '#0066cc' },
        { token: 'attribute.name', foreground: '#cc6600' },
        { token: 'attribute.value', foreground: '#990099' }
      ],
      colors: {
    ...
    monaco.editor.setTheme('html-theme');
  };
  ...
  const handleRunTests = async () => {
    ...
    try {
      ...
      if (!testFile) {
        setOutput(prev => prev + '❌ No HTML test file found');
        setIsRunning(false);
        return;
      }
      ...
      try {
        ...
        setOutput(prev => prev + `❌ HTML test setup error: ${testSetupError.message}\n`);
        setIsRunning(false);
        return;
      }
      ...
      if (window.exerciseTest && window.exerciseTest.runTests) {
        ...
        console.error('HTML test setup failed. Available on window:', Object.keys(window).filter(k => k.includes('exercise')));
      }
      
    } catch (error) {
      setOutput(prev => prev + '\n❌ Test error: ' + error.message);
      console.error('HTML test execution error:', error);
    } finally {
      setIsRunning(false);
    }
  };
